...
index_dict_template: Dict[str, Dict[str, Any]] = {
    "atom": {
        "buffer_lines": 2,
        "start_key": ATOM_BLOCK_START,
        "end_key": ATOM_BLOCK_END,
        "start": None,
        "end": None,
        "block": None,
        "sep_col": 2,
    ...
    "bond": {
        "buffer_lines": 5,
        "start_key": BOND_BLOCK_START,
        "end_key": BOND_BLOCK_END,
        "start": None,
        "end": None,
        "block": None,
        "sep_col": 3,
    ...
    "angle": {
        "buffer_lines": 5,
        "start_key": ANGLE_BLOCK_START,
        "end_key": ANGLE_BLOCK_END,
        "start": None,
        "end": None,
        "block": None,
        "sep_col": 4,
    ...
    "improper": {
        "buffer_lines": 4,
        "start_key": IMPROPER_BLOCK_START,
        "end_key": IMPROPER_BLOCK_END,
        "start": None,
        "end": None,
        "block": None,
        "sep_col": 5,
    ...
    "dihedral": {
        "buffer_lines": 5,
        "start_key": DIHEDRAL_BLOCK_START,
        "end_key": DIHEDRAL_BLOCK_END,
        "start": None,
        "end": None,
        "block": None,
        "sep_col": 5,
...
def find_line_index(file_name: str, keyword: str, start_from: int = 0) -> int:
...
def map_index(
    ...
    for j, line in enumerate(block):
        line_elements = (
            line.split()[0:sep_num] if reverse == 0 else line.split()[sep_num:]
        )
        new_element_list = []
        ...
        leftover = (
            SEPARATOR.join(line.split()[sep_num:])
            if reverse == 0
            else SEPARATOR
            + SEPARATOR.join(line.split()[0:sep_num])
            + SEPARATOR
        )
        index_dict[dof]["block"][j] = (
            SEPARATOR + SEPARATOR.join(new_element_list) + SEPARATOR + leftover
            if reverse == 0
            else leftover + SEPARATOR.join(new_element_list)
        )
    return index_dict


def write_map_file(file_name: str, index_dict: Dict[str, Dict[str, Any]]) -> None:
...
def pop_dict(
    index_dict: Dict[str, Dict[str, Any]], org_mtb_file: str, map_dict: Dict[str, str]
) -> Dict[str, Dict[str, Any]]:
    """Populate a dictionary with mapped indices from an MTB file."""
    for i, dof in enumerate(index_dict.keys()):
        print("Degree of Freedom:", dof)
        start_index = (
            find_line_index(org_mtb_file, index_dict[dof]["start_key"])
            + index_dict[dof]["buffer_lines"]
        )
        ...
        index_dict = map_index(
            index_dict[dof]["block"], sep_num, map_dict, index_dict, dof
        )  # Mapping the indices
        if "sep_col2" in index_dict[dof]:
            sep_num = index_dict[dof]["sep_col2"] - 1
            index_dict = map_index(
                index_dict[dof]["block"], sep_num, map_dict, index_dict, dof, reverse=1
            )
    return index_dict
...
if __name__ == "__main__":
    ...
    if os.path.exists(org_mtb_file):
        index_dict = pop_dict(copy.deepcopy(index_dict_template), org_mtb_file, map_dict)
